...
import static me.matl114.matlib.Algorithms.Algorithm.TransformationUtils.ZERO_VEC;
...
public interface TransformApplicable {
    ...
    default void appendTransformation(Vector3f rotationAxis, float theta, float scaledC, Vector3f b, boolean forceEntityUpdate){
        appendTransformation(new LCTransformation(fromAxisAngle(rotationAxis,theta), scaledC, v2q(b)), forceEntityUpdate);
    }
    ...
    default void applyTransformation(Vector3f rotationAxis, float theta, float scaledC, Vector3f b, boolean forceEntityUpdate){
        applyTransformation(new LCTransformation(fromAxisAngle(rotationAxis,theta), scaledC, v2q(b)),forceEntityUpdate);
    }
