...
public class RegistryImpl<T> extends AbstractRegistry<T> implements Registry<T> {
    public RegistryImpl(String namespace, String name, boolean restrictContent){
        super(namespace, name, new Reference2ReferenceOpenHashMap<>(64), new Object2ReferenceOpenHashMap<>(64), new Object2ReferenceOpenHashMap<>(64));
        ...
        if(this.restrict)
            this.restrictlyContentMap = new WeakHashMap<>(16);
    }
    ...
    protected void validateNamespace(NamespacedKey key){
        if(!Objects.equals(namespace(), key.getNamespace())){
            throw new IllegalStateException("Adding different namespaced key!(This register is in namespace "+namespace +" while adding " + key);
        }
    }
    ...
    public final Content<T> register(NamespacedKey namespacedKey, T value) {
        ...
        if(this.restrict){
            if((valueHolder =resolveRestrictContent(value)) == null){
                throw new IllegalArgumentException("Registry is restricted! Fail to find registryContent for value "+ value);
            }
            Preconditions.checkArgument(Objects.equals(valueHolder.keyStr(), namespacedKey.getKey()), "Registry is restricted ! value's key mismatch");
        }else {
    ...
    protected boolean unregisterInChildNamespace(NamespacedKey key, T value, Content<T> content){
        throw new IllegalArgumentException("This registry does not have child namespace, so key:"+ key+ " with value"+ value +" is invalid argument here!");
    }
    ...
    public Content<T> createRegistryContent(String rawName, T newValue) {
        ...
        if(!restrict){
            ...
            validateWrite();
            return this.restrictlyContentMap.computeIfAbsent(newValue, newV-> (Content<T>) new RegistryContent<>(rawName, newV));
        }
    }
