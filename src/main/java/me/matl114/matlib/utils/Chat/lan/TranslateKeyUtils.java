...
public class TranslateKeyUtils {
    private static final ThreadLocal<ItemMeta> TEMPLATE_STACK = ThreadLocal.withInitial(()->new ItemStack(Material.STONE).getItemMeta());
    ...
    public static String getMaterialTranslateDefault(Material mat){
        ...
        String key = type + "." + mat.getKey().getNamespace() + "." + mat.getKey().getKey();
    ...
    public static String getEnchantmentTranslationDefault(Enchantment enchantment){
        return translateKeyToDefault("enchantment.minecraft." + enchantment.getKey().getKey());
    }
