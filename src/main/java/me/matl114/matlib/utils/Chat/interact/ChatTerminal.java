...
public abstract class ChatTerminal implements InputManager.InputListener {
    ...
    @Override
    public final InputManager.Result onChat(Player player, AtomicReference<String> message, Set<Player> recipients, boolean isCancel) {
        if(!enabled){
        ...
        if("exit".equals(msg)){
            ...
            return onExecution(player, msg, recipients)? InputManager.Result.REJECT_AND_NOT_REMOVE:InputManager.Result.ACCEPT_AND_NOT_REMOVE;
        }
    }
