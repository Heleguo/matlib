...
public class DynamicCodeTests implements TestCase {
    MethodHandle methodHandle = new InitializeSafeProvider<>(()->{
        return MethodHandles.privateLookupIn(DemoFinal.class, MethodHandles.lookup()).unreflect(DemoFinal.class.getDeclaredMethod("getBoolean"));
    }) .v();
...
//        CallSite callSite = LambdaMetafactory.metafactory(MethodHandles.lookup(), "run", MethodType.methodType(Runnable.class,DemoFinal.class), MethodType.methodType(void.class), MethodHandles.lookup().unreflect(method), MethodType.methodType(void.class));
...
//        CallSite callSite1 = LambdaMetafactory.metafactory(MethodHandles.privateLookupIn(DemoFinal.class, MethodHandles.lookup()), "getAsBoolean", MethodType.methodType(BooleanSupplier.class, DemoFinal.class), MethodType.methodType(boolean.class),MethodHandles.privateLookupIn(DemoFinal.class, MethodHandles.lookup()).unreflect(method1), MethodType.methodType(boolean.class));
//        Debug.logger("test private lambda access");
        Function<DemoFinal, BooleanSupplier> lambdaBinder2 = LambdaUtils.createLambdaBinding(BooleanSupplier.class, method1);
        BooleanSupplier supplier = lambdaBinder2.apply(testObj);

        Assert(supplier.getAsBoolean());
        Debug.logger(supplier, supplier.getClass().getClassLoader(), supplier.getClass().getPackage());
        Debug.logger(ReflectUtils.getAllFieldsRecursively(supplier.getClass()), ReflectUtils.getAllMethodsRecursively(supplier.getClass()));
        Debug.logger("test static lambda");

        Supplier<DemoFinal> lambda3 = (Supplier<DemoFinal>) LambdaUtils.createLambdaForStaticMethod(Supplier.class, DemoFinal.class.getDeclaredMethod("ins"));
        AssertNN(lambda3.get() );
        Debug.logger("test dynamic binding");
        Predicate<DemoFinal> lambda4 = (Predicate<DemoFinal>)LambdaUtils.createLambdaForMethod(Predicate.class, DemoFinal.class.getDeclaredMethod("getBoolean"));
...
//        Method methodPackaged =testClass.getDeclaredMethod("nextEntityId");
        Method methodPackaged = ReflectUtils.getAllMethodsRecursively(testClass)
            .stream()
        ...
        BooleanSupplier nextId = LambdaUtils.createLambdaBinding(BooleanSupplier.class, methodPackaged).apply(handle);
        Debug.logger(nextId.getClass().getModule() , testClass.getModule(), nextId.getClass().getModule() ==  testClass.getModule(), nextId.getClass().getModule() == this.getClass().getModule());
