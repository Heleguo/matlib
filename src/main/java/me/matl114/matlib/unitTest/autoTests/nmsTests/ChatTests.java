...
public class ChatTests implements TestCase {
    ...
    public void test_chatComp() throws Throwable{
        ...
        for (int i=0; i< 5;++i){
            ...
            Object val = CraftUtils.getDisplayNameHandle().get(meta);
            if(val instanceof String){
                AssertEq(meta.getDisplayName(), ChatUtils.serializeToLegacy(NMSChat.CHATCOMPONENT.fromJson((String) val)));
            }
        }
    ...
    public void test_cpfr() throws Throwable{
        ...
        component = component.append(Component.text("sub2").append(Component.text("subsub1")).append(Component.text("subsub2")));

        Bukkit.getServer().sendMessage(component);
        ItemMeta meta = new ItemStack(Material.STONE).getItemMeta();
        meta.displayName(component);
        Debug.logger(meta.getDisplayName());
        Iterable<?> adventure = CHATCOMPONENT.newAdventure(component);
        Iterable<?> nms = CHATCOMPONENT.deepConverted(adventure);
        Debug.logger(nms);
        Debug.logger(CHATCOMPONENT.toJson(nms));
    ...
    public void test_itemNameAndLoreAPI() throws Throwable{
        ...
        Assert(ITEMSTACK.hasLore(nms));
        Assert(ITEMSTACK.hasCustomHoverName(nms));
        Debug.logger(ITEMSTACK.getHoverName(nms));

        Debug.logger(ITEMSTACK.getLoreView(nms, false));
        ListMapView<?,Iterable<?>> lorelist = ITEMSTACK.getLoreView(nms,false);
        lorelist.add(CHATCOMPONENT.append(CHATCOMPONENT.empty(), CHATCOMPONENT.literal("这是第四行lore,惊不惊喜意不意外")));
