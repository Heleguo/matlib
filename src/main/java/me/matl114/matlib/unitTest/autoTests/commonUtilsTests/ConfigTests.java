...
public class ConfigTests implements TestCase {
    ...
    public void test_config() throws Throwable {
        ...
        try(InputStream file = FileUtils.readResource(filePath + ".yml")){
            ...
            config1 = new io.github.thebusybiscuit.slimefun4.libraries.dough.config.Config(null, ResourceUtils.loadInternalConfig(MatlibTest.class, filePath)) ;
        }catch (Throwable e){
        ...
        for (var keyEntry : IterUtils.fastEnumerate(keys)){
            try{
                ...
                Debug.logger("Error:", keyEntry.getIndex(), keyEntry.getValue(), config.getLeaf(keyEntry.getValue()).get(), config1.getValue(keyEntry.getValue()));
                return;
            }
        }
    }
