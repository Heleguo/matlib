...
public class WorldUtilTests implements TestCase {
    ...
    public void test_copyBlockstate()throws Throwable{
        ...
        Assert( ScheduleManager.getManager().getScheduledFuture(()->{
            location.getBlock().setType(Material.CHEST);
            VersionedWorld.getInstance().copyBlockStateTo(location.getBlock().getState(false), targetLocation.getBlock());
            return targetLocation.getBlock().getState(false);
        },0, true).get() instanceof Chest);
    }
}
