...
public class NMSPlayerTest implements TestCase {
    ...
    public void test_test(CommandSender sender) throws Throwable{
        ...
        NMSItem.ITEMSTACK.setItem(nms, NMSCore.REGISTRIES.getValue(BuiltInRegistryEnum.ITEM, NMSCore.NAMESPACE_KEY.newNSKey("minecraft","myitem")));
    ...
    public void test_translation(CommandSender sender)throws Throwable{
       ...
       // Debug.logger("before set",p.getInventory().getItemInMainHand());
        ...
        Component name = Component.empty().append(Component.text("并非翻译部分名字").style(commonStyle)).append(Component.translatable("me.matlib.test.translation.path1").append(Component.text("并非翻译部分名字2").style(commonStyle)));
        meta1.displayName(name);
        Component lore = Component.empty().append(Component.text("并非翻译部分lore").style(commonStyle)).append(Component.translatable("me.matlib.test.translation.path2")).append(Component.text("并非翻译部分lore2").style(commonStyle));
