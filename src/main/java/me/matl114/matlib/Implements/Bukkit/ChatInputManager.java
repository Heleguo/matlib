    ...
    private void checkInput(Cancellable e, Player p, String msg,Set<Player> recipients) {
        ...
        if (callbacks != null && !callbacks.isEmpty()) {
            do{
                ...
                if(last != null ){
                    e.setCancelled(true);
                    TaskRequest.run(last.getRunningRequest(),()->last.onChat(p,msg,recipients),pl);
                    return;
                }
        ...
        registerInputListener(new InputListener() {
            public Result onChat(Player player, AtomicReference<String> message, Set<Player> recipients, boolean isCancel) {
                try{
                    ...
                    Debug.logger("Error while inputListener"+returnCode+("(Class:")+returnCode.getClass()+") handle the input message:");
                    Debug.logger(e);
