...
public class MachineRecipeUtils {
    ...
    public static ItemStack[] stackIn(ItemStack[] items){
        ...
        for(int i = 0; i < items.length; i++){
            ...
            if(ac.getItem() instanceof AbstractItemStack abs&&!abs.canStackWithMatch()){
                stacks.add(ac);
                continue;
            }
            __flag=false;
        ...
        for(ItemCounter stackCounter : stacks){
            if(stackCounter.getItem() instanceof AbstractItemStack abs&&!abs.canSetAmount()){
                abs.setRealAmount(stackCounter.getAmount());
                result[cnt] = (ItemStack) abs;
...
//            }
    ...
    public static ItemStack[] makeValidMGInput(ItemStack[] input){
        ...
        for(int i = 0; i < input.length; i++){
            if(input[i]==null)continue;
            if(input[i] instanceof AbstractItemStack abs&& abs.isVirtual())continue;
            it.add(input[i]);
        }
