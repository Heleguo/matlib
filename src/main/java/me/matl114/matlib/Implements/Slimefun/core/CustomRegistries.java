package me.matl114.matlib.Implements.Slimefun.core;
...
public class CustomRegistries implements   Manager {
        ...
        public void tryPut(String key,Object val){
            if(!lock&&clazz.isInstance(val)){
                try{
                    ...
                    Debug.logger("Error while trying to register "+key+":"+val,"Error message: "+e.getMessage());
                }
            }
        }
    ...
    public <W extends Object> W getInternal(String key, Class<W> clazz) {
        Registry<W> re=(Registry<W>) registries.stream().filter(r->r.clazz==clazz).findFirst().orElse(null);
        if(re==null){
            return null;
        }else {
            return re.get(key);
    ...
    public <W extends Object> boolean removeInternal(String key, Class<W> clazz) {
        Registry<?> re=registries.stream().filter(r->r.clazz==clazz).findFirst().orElse(null);
        if(re!=null){
    ...
    private Registry<CustomRecipeType> customRecipeType=new Registry<>(CustomRecipeType.class)
            .predicate((val)->true)
            .register();
    private Registry<CustomSlimefunItem> customSlimefunItem=new Registry<>(CustomSlimefunItem.class)
            .predicate((val)->true)
            .register();
