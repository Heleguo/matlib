...
public class RegistryUtils {
    private static final Reference2ReferenceMap<Registry<?>,Iterable<?>> bukkitRegistryToMinecraftCache = new Reference2ReferenceOpenHashMap<>();

    public static Iterable<?> toMinecraftRegistry(Registry<?> bukkitRegistry){
        return bukkitRegistryToMinecraftCache.computeIfAbsent(bukkitRegistry, CraftBukkit.REGISTRY::minecraftRegistryGetter);
    }
    public static  <B extends Keyed, M> B minecraftToBukkit(M minecraft, Registry<B> bukkitRegistry){
        ...
        return bukkitRegistry.get(fromMinecraftNSK(REGISTRIES.getKey(minecraftRegistry, minecraft)));
        //return CraftBukkit.REGISTRY.minecraftToBukkit(minecraft, NMSCore.REGISTRIES.key(minecraftRegistry), bukkitRegistry);
    }
    public static Object getByKey(Object registry, String key){
        return REGISTRIES.getOptional(registry, resourceLocation(key)).orElseThrow();
    }
    public static Object getByKeyOrNull(Object registry, String key){
        return REGISTRIES.getValue(registry, resourceLocation(key));
    }

    public static Object getResourceLocation(Object registry, Object value){
        return REGISTRIES.getKey(registry, value);
    }

    public static <B extends Keyed, M> Object byKeyBukkitToMinecraft(B bukkit, Object nmsRegistry){
        return REGISTRIES.getOptional(nmsRegistry, fromBukkit(bukkit.getKey())).orElseThrow();
    }

    public static Object validateAsHolder(Object nms, Object nmsRegistry){
        Object val = REGISTRIES.wrapAsHolder(nmsRegistry, nms);
        if(REGISTRIES.isHolderReference(val)){
            return val;
        }else {
    ...
    public static <M> Object enchantmentToMinecraftHolder(Enchantment enchantment){
        if(BuiltInRegistryEnum.ENCHANTMENT != null){
            return validateAsHolder(CraftBukkit.REGISTRY.enchantmentToMinecraft(enchantment), BuiltInRegistryEnum.ENCHANTMENT);//  handledBukkitToMinecraftHolder(enchantment, BuiltInRegistryEnum.ENCHANTMENT);
        }else {
            return validateAsHolder(CraftBukkit.REGISTRY.enchantmentToMinecraft(enchantment), CraftBukkit.REGISTRY.getMinecraftRegistry(RegistryKeyEnum.ENCHANTMENT));
        }
    }

    public static <M> Object attributeToMinecraftHolder(Attribute attribute){
        return validateAsHolder(byKeyBukkitToMinecraft(attribute, BuiltInRegistryEnum.ATTRIBUTE),BuiltInRegistryEnum.ATTRIBUTE);
    }
    ...
    public static NamespacedKey fromMinecraftNSK(Object nms){
        ...
        return new NamespacedKey(NAMESPACE_KEY.getNamespace(nms), NAMESPACE_KEY.getPath(nms));
    }
    ...
    public static Object resourceLocation(String string){
        ...
        if(index == -1){
            return NAMESPACE_KEY.newNSKey("minecraft",string);
        }else {
            return NAMESPACE_KEY.newNSKey(string.substring(0, index), string.substring(index +1));
        }
    }
    public static Object fromBukkit(NamespacedKey key){
        ...
        return NAMESPACE_KEY.fromBukkit(key);
    }
