...
//    CLIENTBOUND_UPDATE_TAGS(true, PacketFlow.S2C, "update_tags"),
    ...
    //if you really need some shit in small version like 1.21.2 or 1.21.0, please use ALL_C2S or ALL_S2C and use instanceof or sth to check class
            ...
            try{
                switch (packetName){
                    case "ClientboundBundleDelimiterPacket":
                        return ObfManager.getManager().reobfClass("net.minecraft.network.protocol.BundleDelimiterPacket");
                    case "ServerboundBlockEntityTagQueryPacket":
                        return ObfManager.getManager().reobfClass("net.minecraft.network.protocol.game.ServerboundBlockEntityTagQuery");
                    case "ServerboundEntityTagQueryPacket":
                        return ObfManager.getManager().reobfClass("net.minecraft.network.protocol.game.ServerboundEntityTagQuery");
                    default:Debug.logger(e,"Could not find class name for",fullClassName,"which from", name);
                }
            }catch (Throwable e1){
