...
public interface ItemDataValue {
    ...
    @Note(value = "In version higher than v1_20_R4, we don't support multiple path anyMore, path must be one of DataComponentTypes",extra = {
        "If you want to set custom NBT in higer version, use ItemStackHelper.getCustomedNbtView"
    })
    ...
    @Note(value = "This method returns a ready only ItemDataValue where you can only use get and remove, it may takes lower costs")
