...
//
    ...
    public void visitClickEvent(ClickEvent.Action clickEvent, List rawData) {
        ...
        if(isConst){
            String value = (String) rawData.stream().map(i->{
            ...
            eventProcessor.add((arg, sty)->{
                String value = (String) rawData.stream().map(i->{
                    if (i instanceof BaseTypeAST bbbb)return bbbb.isPlaceholder()? arg.getAsString( bbbb.getRaw()): bbbb.getRaw();
                    else return String.valueOf(i);
                }).collect(Collectors.joining());
        ...
        holder.setValue( new NMSComponentBuilder((argfunc)->{
            ...
            if(thi.isConst()){
                ...
                if(constChild){
                    ...
                    childProcessor.add(((argumentProvider, componentBuilder) -> componentBuilder.siblings().add(sub)));
                }
            }else {
                constChild = false;
                childProcessor.add(((argumentProvider, componentBuilder) -> componentBuilder.siblings.add(argfunc.apply(argumentProvider).toImmutable())));
            }
        }));
        return holder.get();
    }
