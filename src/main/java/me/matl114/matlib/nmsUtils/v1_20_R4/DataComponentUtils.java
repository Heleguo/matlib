...
public class DataComponentUtils {
    ...
    public static Codec<Object> getTypeCodec(Object type){
        Codec<Object> codecValue = Env1_20_R4.DATA_TYPES.getDataTypeCodec(type); //ComponentCodecEnum.DATA_COMPONENT_CODEC_MAP.get(type);
        if(codecValue == null){
            throw new IllegalArgumentException("This DataComponentType is temporary and can not be saved by this method");
        }
        return codecValue;
    }

    public static String typeId(Object string){
        return RegistryUtils.getResourceLocation(BuiltInRegistryEnum.DATA_COMPONENT_TYPE, string).toString();
    }
