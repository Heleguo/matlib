...
public class Codecs {
    public static <S extends Enum<?>> Codec<S> enumLowerCase(Class<S> enumClass){
        ...
        Function<S, String> nameGetter = createToNameMap(enumConstants, Enum::name, name->name.toLowerCase(Locale.ROOT));
        Function<String, S> nameLookup = createNameLookup(enumConstants, Enum::name, name->name.toLowerCase(Locale.ROOT));
        ToIntFunction<S> indexLookup = Enum::ordinal;
        return new StringCodec<>(enumConstants, nameGetter, nameLookup, indexLookup);
    }
    public static <T> Function<T, String> createToNameMap(T[] values,  Function<T, String> nameGetter, Function<String, String> valueNameTransformer){
        Map<T,String> map = Arrays.stream(values)
            .collect(Collectors.toMap(value -> (T)value, value -> valueNameTransformer.apply(nameGetter.apply(value))));
        return name -> name == null ? null : map.get(name);
    }
    public static <T> Function<String, T> createNameLookup(T[] values, Function<T, String> nameGetter, Function<String, String> valueNameTransformer) {
        Map<String, T> map = Arrays.stream(values)
            .collect(Collectors.toMap(value -> valueNameTransformer.apply(nameGetter.apply(value)), value -> (T)value));
        return name -> name == null ? null : map.get(name);
    }
        ...
        return Codec.INT
            .flatXmap(
                ...
                element -> {
                    ...
                    return j == errorRawId ? DataResult.error(() -> "Element with unknown id: " + element) : DataHelper.A.I.success(j);
                }
            );
    }
